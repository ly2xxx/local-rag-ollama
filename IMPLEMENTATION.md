...
│   ├── config.py                       # Central environment variable settings (.env)
│   ├── agent.py                        # High-level AgenticRAGHelper facade for Streamlit
│   ├── core.py                         # [IMPLEMENTED] AgentCore with LangGraph ReAct loop
│   ├── graph_display.py                # [IMPLEMENTED] LangGraph topology visualizer (Mermaid/PNG)
...
├── pyproject.toml                      # Updated with langgraph, langgraph-checkpoint-redis, redis, python-dotenv
...
### B. Visual Graph Topology Renderer (`agentic_rag/graph_display.py`)
- Matches the visualization style in `langgraph_ollama/app.py`.
- Extracts Mermaid syntax via `graph.draw_mermaid()`.
- Caches rendered diagram PNGs under `.cache/graph-png/` and falls back to native Mermaid blocks if offline.
- Renders collapsible topology view directly inside Streamlit Tab 3.

### C. 4. Tools (`agentic_rag/tools.py`)
...
### D. 3. Short-term Memory & Redis Checkpointing (`agentic_rag/memory/short_term.py`)
...
### E. Helper Facade (`agentic_rag/agent.py`)
...
2. **Visual Graph Topology Expander**: Visual diagram of the LangGraph state machine (`__start__ -> agent <-> tools -> __end__`) and raw Mermaid source.
3. **Document Ingestion**: Multi-file PDF uploader dedicated to Agentic RAG.
4. **Session / Thread ID Selector**: Configurable `thread_id` to demonstrate multi-turn persistence across sessions.
5. **Interactive Chat & Scratchpad Expander**: Shows conversation messages alongside collapsible **"🔍 Inspect Scratchpad & Tool Executions"** showing intermediate tool calls, inputs, and observations.
6. **Reset Button**: Resets current thread state and document stores.
